...
public class MainInfoAboutProductPage extends BasePage {
    ...
    @FindBy(xpath = "//li[contains(@class,'colors')]/span")
    private List<WebElement> colorsOfProductText;
    ...
    public boolean checkColorsProduct() {
        ...
        if (!(colorsOfProductText.get(0).getText().contains(colorTitle.get(0).getText()) && descriptionTitleText.getText().contains(colorTitle.get(0).getText()))) {
            return false;
        }
        for (int i = 1; i < colorsOfProductLinks.size(); i++) {
            colorsOfProductLinks.get(i).click();
            WaitUtils.explicitWait(getWebDriver(), ExpectedConditions.visibilityOfAllElements(colorsOfProductLinks));
            if (!(colorsOfProductText.get(i).getText().contains(colorTitle.get(0).getText()) && descriptionTitleText.getText().contains(colorTitle.get(0).getText()))) {
                return false;
            }
        }
        return true;
